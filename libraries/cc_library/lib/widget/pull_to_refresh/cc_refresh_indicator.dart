...
class CcCustomRefreshIndicator extends StatefulWidget {
  ...
  @override
  CcCustomRefreshIndicatorState createState() => CcCustomRefreshIndicatorState();
}

class CcCustomRefreshIndicatorState extends State<CcCustomRefreshIndicator> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    _customRefreshIndicatorController = widget.controller ?? CcIndicatorController._();
  ...
  bool _checkCanHide(ScrollNotification notification) {
    if (notification is ScrollUpdateNotification) {
      // print(
      //     "notification.dragDetails.globalPosition. ${(notification as ScrollUpdateNotification).dragDetails?.delta.dy}  controller ${controller.state}");
  ...
  Future<void> _hideAfterRefresh() async {
    ...
    await _animationController.animateTo(0.0, duration: widget.indicatorFinalizeDuration);
